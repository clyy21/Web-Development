...
import { Table, Input, Button, Space, DatePicker, Divider, Layout } from "antd";
...
class UserStats extends React.Component {
    ...
    this.columns = [
      ...
      },
      {
        ...
        render: (record) => {
          return (
            "￥" +
            record.round(2, bigDecimal.RoundingModes.HALF_EVEN).getValue()
          );
        },
      },
    ];
  ...
  onSearchByTime = () => {
    ...
    for (let i = 0; i < len; i++) {
      if (
        this.state.order[i].time >= this.state.start_t &&
        this.state.order[i].time <= this.state.end_t
      ) {
        if (!result.has(this.state.order[i].userId)) {
          result.set(
            this.state.order[i].userId,
            new bigDecimal(this.state.order[i].price)
          );
        } else {
          tmp = result.get(this.state.order[i].userId);
          result.set(
            this.state.order[i].userId,
            tmp.add(new bigDecimal(this.state.order[i].price))
          );
        }
      }
    }
    ...
    Result.sort(function (a, b) {
      return bigDecimal.compareTo(b[1].value, a[1].value);
    });
    console.log(Result);
    ...
    for (let i = 0; i < tmp_len; ++i) {
      result1.push({
        value: obj[i].price
          .round(2, bigDecimal.RoundingModes.HALF_EVEN)
          .getValue(),
        name: obj[i].name,
      });
    }
  ...
  render() {
    ...
    return (
      <div className="container">
        <div className="topblankspace">
          <Navbar />
          <Layout className="adminview-layout">
            <SideBar selected={"5"} />
            <div className="booktable-wrapper">
              <div className="booktable-container userorder-table">
