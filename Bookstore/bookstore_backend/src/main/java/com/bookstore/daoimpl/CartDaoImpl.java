...
//  @Transactional(propagation = Propagation.REQUIRES_NEW)
  ...
  public List<CartItem> addOrder(Integer userId) {
    System.out.println("addOrder dao executed.");
    if (userId == null) {
      return null;
    }
//    deleteOrderCache(userId);
    ...
    orderRepository.addOrder(userId, nowDate, myPrice); //加订单
    System.out.println("addOrder dao completed.");
//    int result = 10 / 0;
    return myCart;
  }
...
//  @Transactional
 @Transactional(propagation = Propagation.REQUIRES_NEW)
  public void addOrderItem(Integer userId) {
    ...
    System.out.println("addOrderItem dao executed.");
    Order myOrder = orderRepository.getMaxOrder();
    List<CartItem> myCart = cartRepository.getRealCartItems(userId);
    for (int i = 0; i < myCart.size(); i++) {
      CartItem myItem = myCart.get(i);
    ...
    System.out.println("addOrderItem dao completed.");
//    int result = 10 / 0;
  }
...
//  @Transactional(propagation = Propagation.REQUIRES_NEW)
  public void deleteCart(Integer userId) {
    ...
    System.out.println("deleteCart dao executed.");
    cartRepository.submitCart(userId);  //清空购物车
    System.out.println("deleteCart dao completed.");
//    int result = 10 / 0;
  }
...
//  @Transactional(propagation = Propagation.REQUIRES_NEW)
  public void updateBookInventory(Integer userId, List<CartItem> myCart) {
    ...
    System.out.println("updateBookInventory dao executed.");
    //订单项为空时不应该加订单
    if (myCart == null) {
      return;
    }
    ...
    System.out.println("updateBookInventory dao completed.");
//    int result = 10 / 0;
  }
}
